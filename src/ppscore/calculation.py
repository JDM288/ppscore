...
def _calculate_model_cv_score_(
    ...
    scores = cross_val_score(
        model, feature_input, target_series.astype("float"), cv=cross_validation, scoring=metric
    )
...
def _mae_normalizer(df, y, model_score, **kwargs):
    ...
    baseline_score = mean_absolute_error(df[y].astype("float"), df["naive"].astype("float"))  # true, pred
